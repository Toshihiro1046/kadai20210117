...
echo '<div class="w-75">';
    ...
    echo '<tr>';
    echo '<th class = "table1">名前</th><th class = "table1">メールアドレス</th>';
...
if ($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    $view = h($result['name']) . ' / ' . h($result['email']) . ' / ' . 
            h($result['evaluation']) . ' / ' . h($result['movietype']) . ' / ' .
            h($result['naiyou']) . ' / ' . '<a href="detail.php?id=' . $result['id'] . '">' . h($result['indate']) . '</a>' .
